...
// convenience method
func CreateOrOpen(path string, sync bool) (*Datastore, error) {
	err := Create(path)
...
func (fs *Datastore) marshal(key datastore.Key, value []byte) []byte {
	keyLength := uint16(len(key.Bytes()))

	data := make([]byte, 0, 2+int(keyLength)+len(value))
	// two bytes key length header
	data = append(data, byte(keyLength>>8), byte(keyLength))
...
func (fs *Datastore) unmarshal(data []byte) (key datastore.Key, value []byte, err error) {
	if len(data) < 2 {
		err = fmt.Errorf("data length is smaller than 2: %d", len(data))
		return
	}
	keyLength := int(data[0])<<8 + int(data[1])
	if keyLength == 0 {
		err = errors.New("keyLength is zero")
		return
	}
	if len(data) < 2+keyLength {
		err = fmt.Errorf("data length is smaller than keyLength+2: %d", len(data))
		return
	}
	keyBytes := data[2 : 2+keyLength]
	key = datastore.NewKey(string(keyBytes))
	value = data[2+keyLength:]
	return
}

// encode returns the directory and file names for a given key
func (fs *Datastore) encode(key datastore.Key) (dir, file string) {
	h := sha1.New()
	_, err := h.Write(key.Bytes())
	if err != nil {
		panic(err)
	}
	sum := hex.EncodeToString(h.Sum(make([]byte, 0)))
	dir = filepath.Join(fs.path, sum[:2])
	file = filepath.Join(dir, sum[2:])
	return dir, file
}
...
func (fs *Datastore) doPut(key datastore.Key, val []byte) error {
	...
	if _, err := tmp.Write(fs.marshal(key, val)); err != nil {
		return err
	}
	if fs.sync {
...
func (fs *Datastore) putMany(data map[datastore.Key][]byte) error {
	...
	for key, value := range data {
		...
		if _, err := tmp.Write(fs.marshal(key, value)); err != nil {
			return err
		}
	}
...
func (fs *Datastore) walk(ctx context.Context, path string, qrb *query.ResultBuilder) error {
	...
	for _, fn := range names {
		...
		data, err := readFile(filepath.Join(path, fn))
		if err != nil {
			result.Error = err
		}
		key, value, err := fs.unmarshal(data)
		if err != nil {
			return err
		}
